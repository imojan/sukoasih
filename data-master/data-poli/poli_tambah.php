...
$query = mysqli_query($koneksi, "SELECT id_poli FROM tb_poli ORDER BY id_poli DESC LIMIT 1");
$data_kode = mysqli_fetch_assoc($query);

$no_akhir = (isset($data_kode['id_poli'])) ? (int)substr($data_kode['id_poli'], -3) : 0;
$no_baru = $no_akhir + 1;
$kode_baru = str_pad($no_baru, 3, "0", STR_PAD_LEFT);
$id_poli_baru = "P" . $kode_baru;
...
<head>
    ...
    <title>Suko Asih | Data Poli</title>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="">
                                <form class="ml-4" method="post" enctype="multipart/form-data">
                                    <div class="form-group row">
                                        <div class="col-sm-4">
                                            ...
                                            <input type="text" class="form-control" name="id_poli" value="<?= $id_poli_baru; ?>" readonly>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <div class="col-sm-4">
                                ...
                                <?php
                                    if (isset($_POST['save'])) {
                                        $nm_poli = $koneksi->real_escape_string($_POST['nm_poli']);
                                        $koneksi->query("INSERT INTO tb_poli (nm_poli) VALUES ('$nm_poli')");

                                        $id_poli = $koneksi->insert_id;
                                        $formatted_id_poli = str_pad($id_poli, 3, '0', STR_PAD_LEFT);

                                        echo "<script>alert('Data Tersimpan! ID Poliklinik: P$formatted_id_poli');</script>";
