...
// Amankan input
$id_dokter = isset($_GET['id_dokter']) ? mysqli_real_escape_string($koneksi, $_GET['id_dokter']) : null;

// Cek apakah parameter ada
if (!$id_dokter) {
    echo "<div class='alert alert-danger'>ID Dokter tidak ditemukan di URL.</div>";
    exit;
}

// Query gabungan dokter dan poli
$ambil = $koneksi->query("SELECT * FROM tb_dokter 
...
if ($ambil && $ambil->num_rows > 0) {
    ...
    // Ambil id_poli dari hasil
    $id_poli = $pecah['id_poli'];

    // Ambil nama poli (boleh, tapi sebenarnya sudah bisa dari JOIN)
    $q_poli = mysqli_query($koneksi, "SELECT nm_poli FROM tb_poli WHERE id_poli = '$id_poli'");
    $data_poli = mysqli_fetch_assoc($q_poli);
    $nm_poli = $data_poli ? $data_poli['nm_poli'] : '-';

} else {
    echo "<div class='alert alert-danger'>Data dokter tidak ditemukan untuk ID: $id_dokter</div>";
    exit;
}
                                ...
                                <form class="ml-4" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-4">
                                            ...
                                            <input type="text" class="form-control" name="id_poli" value="<?php echo $pecah['nm_poli'] ?>" readonly>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-4">
                                            ...
                                            <input type="text" class="form-control" name="telp_dokter" value="<?php echo $pecah['telp_dokter'] ?>" readonly>
                                        </div>
                                    </div>
